...
This is the **differentiation move #2** artefact: a real measurement table that
shows the KV-cache speedup at several context lengths. Run on the local CPU
for a smoke / sanity check; the production numbers belong on a T4 in Phase F.

Usage:
    python scripts/benchmark_kv_cache.py
    python scripts/benchmark_kv_cache.py --ctx-lengths 128 256 --warmup 1

Writes ``docs/results/kv_cache_bench.md`` with a Markdown table.
"""
...
logger = logging.getLogger("forge_llm.bench_kv_cache")

_DEFAULT_CTX_LENGTHS = [128, 512, 2048]
_DEFAULT_OUT = Path("docs/results/kv_cache_bench.md")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.small:
        ...
        preset["max_seq"] = max(args.ctx_lengths)
    ...
    for ctx in args.ctx_lengths:
        max_new = min(ctx, config.max_seq - 4)  # leave a small prompt budget
        logger.info("ctx=%d (max_new=%d)", ctx, max_new)

        for _ in range(args.warmup):
            _bench(model, tokenizer, "the", max_new=8, use_cache=True, device=args.device)

        nocache = _bench(model, tokenizer, "the", max_new=max_new, use_cache=False, device=args.device)
        cached = _bench(model, tokenizer, "the", max_new=max_new, use_cache=True, device=args.device)
